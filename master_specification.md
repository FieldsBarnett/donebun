...
*   **Task Details & Interaction**:
    ...
    *   **Repeating Tasks**: Robust, backend-driven recurring schedules using two distinct architectures to ensure data integrity and prevent "ghosting":
        *   **Scheduled (iCal / RFC-5545 Inspired)**: Repeats on a set cadence regardless of completion (e.g., pay bills every month).
            ...
            *   *Edge Cases & Mutations*: To prevent moved tasks from reappearing in their original slots ("ghosting"), materialized exceptions store an `originalDueDate` field. Deleting a virtual occurrence or the **root task itself as a "single instance"** adds that date to an `excludedDates` array on the root task. This allows the series to persist while hiding that specific occurrence from the UI, keeping the database free of tombstone records.
            *   *Deletion UX*: When a user deletes a recurring task, they are prompted to choose between deleting "This occurrence only", "This and following" (which splits the series), or "All occurrences" (which performs a hard delete of the root and all materialized exceptions).
            *   *Series Splitting*: Editing or deleting "This and following occurrences" sets an `endDate` on the old root and spawns a new root from that date forward, preserving history.
...
*   **Sync Architecture**:
    ...
    *   **Background & Immediate Updates**: Syncs are triggered automatically when a user views their calendar, via scheduled Convex Crons (every two hours), and **immediately** upon connecting a new Google account or re-enabling a previously disabled calendar sync.
