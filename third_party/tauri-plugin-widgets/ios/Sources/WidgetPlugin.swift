...
class WidgetPlugin: Plugin {
    ...
    /// and mirrors into App Group UserDefaults (widget extension reads both).
    private func writeDataMap(_ map: [String: String], group: String) -> Bool {
        let appGroup = resolveAppGroup(group)
        var wroteSomething = false

        if let url = dataFileURL(group: group) {
            do {
                let data = try JSONSerialization.data(withJSONObject: map, options: [.prettyPrinted, .sortedKeys])
                try data.write(to: url, options: .atomic)
                wroteSomething = true
            } catch {
                // Fall through to UserDefaults — some devices fail file writes intermittently.
            }
        }

        // Mirror to UserDefaults so WidgetKit can read even if the shared file write failed.
        // TauriWidgetDataStore.readValue checks suite "widget_data" for both plain and group. IDs.
